[
    ...
    },
    {
        ...
        "func": "var RFIDRequest = msg.payload;\n\nvar hive = global.get('hive');\nvar JMRI_URL = global.get('JMRI_URL');\n\nvar reporter = hive.getReporterName(RFIDRequest.MAC);\nvar location = hive.jmri.getLocationByReporter(reporter.name);\n\nif (hive.empty(location) || location.name === undefined) {\n    /* Backup to use TrainTraxx Information in the event JMRI Reporters were unable to be assigned \n        This will be the case on test versions 4.17.3 and earlier\n    */\n    location = hive.jmri.getLocationByTrainTraxxMAC(RFIDRequest.MAC);\n}\n\nvar tag = hive.getTagName(RFIDRequest.UID);\nvar inventory = hive.jmri.getInventoryByTag(tag.name);\n\n\ninventory.location = location;\n\nif (!hive.empty(location) && !hive.empty(inventory)) {\n    if (location.name !== undefined && !hive.empty(inventory.destination) && !hive.empty(inventory.destination.name)) {\n        /* Need to include a status flag for coming or going */\n        if (!hive.empty(location.track) && !hive.empty(location.track.name) && !hive.empty(inventory.destination.track) && !hive.empty(inventory.destination.track.name)) {\n            if (location.track.name === inventory.destination.track.name) {\n                /* Toggle State */\n                inventory.comment = (inventory.comment === 'arrived') ? 'left' : 'arrived';\n                if (inventory.comment === 'arrived') {\n                    inventory.load = (inventory.load === 'E') ? 'L' : 'E';\n                }\n            } else {\n                inventory.comment = 'enroute';\n            }\n        } else {\n            inventory.comment = (inventory.comment === 'arrived') ? 'left' : 'arrived';\n            if (inventory.comment === 'arrived') {\n                inventory.load = (inventory.load === 'E') ? 'L' : 'E';\n            } else {\n                inventory.comment = 'enroute';\n            }\n        }\n    }\n}\nvar type = inventory.type;\ndelete inventory.type;\nmsg.verb = 'POST';\n\nmsg.payload = {\n    \"type\": type,\n    \"data\": inventory\n};\n\n\nmsg.headers = {'content-type':'application/json'};\nmsg.url = JMRI_URL + 'json/'+ type;\nmsg.topic = type;\nreturn msg;",
        "outputs": 1,
        "noerr": 0,
        "x": 690,
        "y": 640,

[
    ...
    },
    {
        ...
        "func": "var inProbes = msg.payload;\nmsg.payload = [];\nvar inHealth = global.get('Probe_Health');\nfor (var i in inProbes) {\n    var color = 'green';\n    switch (inHealth[inProbes[i].MAC]) {\n        case 404:\n        case 500:\n        case 501:\n            color = \"red\";\n            break;\n    }\n    var successRate = '<i class=\"fas fa-circle\" style=\"color: #ccc;\">N/A</i>';\n    var total = inProbes[i].SUCCESS + inProbes[i].FAILURE;\n    if (total > 0) {\n        var tempRate = Math.Round((inProbles[i].SUCCESS*100)/total);\n        var colorR = (tempRate < 75) ? ((tempRate < 50) ? 'red' : 'orange') : 'green';\n        successRate = '<i class=\"fas fa-circle\" style=\"color: ' + colorR + ';\">' + String(tempRate) +'%</i>';\n    }\n    msg.payload.push({\n       title : '<strong>' + inProbes[i].IP + '</strong><i class=\"fas fa-circle\" style=\"color:' + color + ';\" />' +\n        inProbes[i].MAC +\n        \"<br /><strong>Version: </strong>\" + inProbes[i].VERSION + \"<br />\" +\n        \"Success Rate: \" + successRate,\n       menu : [\"Restart\"],\n       icon_name : 'wifi'\n    });\n}\n\n\nreturn msg;",
        "outputs": 1,
        "noerr": 0,
        "x": 890,
        "y": 440,

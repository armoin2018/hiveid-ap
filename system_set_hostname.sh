...
NEW_HOST=$1
mkdir /opt/hiveid-ap/backups /opt/hiveid-ap/backups/$DATE 2>/dev/null
cp /etc/hostname /etc/dhcpcd.conf /etc/hostapd/hostapd.conf /etc/hosts /opt/hiveid-ap/backups/$DATE/.
